...
class LineChartState extends State<LineChart> {
            ...
            return Scaffold(
                      ...
                      DropdownButton(
                        ...
                        onChanged: (String? newValue) {
                          setState(() {
                            ...
                            int monthNumber =
                                getMonthNumber(dropdownvalueMonth!);
                            int days = getDaysInMonth(
                                int.parse(dropdownvalueYear), monthNumber);

                            //error here
                            spots = List<double>.filled(days, 0,
                                growable: false); //now spots is cleared
                            getSpots().then((value) => spots = value);
                ...
                body: Container(
                    ...
                    child: SfCartesianChart(
                        ...
                        title: ChartTitle(
                            text:
                                'Firedepartment: Year: $dropdownvalueYear   Month: $dropdownvalueMonth    Type: $dropdownvalueType'),
                        tooltipBehavior: TooltipBehavior(enable: true),
                        series: <ChartSeries<AlarmData, String>>[
                          LineSeries<AlarmData, String>(
                            dataSource: data,
                            xValueMapper: (AlarmData alarms, _) => alarms.month,
                            yValueMapper: (AlarmData alarms, _) =>
                                alarms.alarmCount,
                            name: 'Alarms',
                          )
                        ])));
  ...
  Future<List<double>> getSpots() async {
      ...
      for (int i = 0; i < dates.length; i++) {
        if (dropdownvalueType == null || dropdownvalueType == "Alle") {
          if (dates[i].contains('Jan') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[0]++;
            ...
            spotsArray[1]++;
              ...
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[2]++;
          } else if (dates[i].contains('Apr') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[3]++;
          } else if (dates[i].contains('May') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[4]++;
          } else if (dates[i].contains('Jun') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[5]++;
          } else if (dates[i].contains('Jul') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[6]++;
          } else if (dates[i].contains('Aug') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[7]++;
          } else if (dates[i].contains('Sep') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[8]++;
          } else if (dates[i].contains('Oct') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[9]++;
          } else if (dates[i].contains('Nov') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[10]++;
          } else if (dates[i].contains('Dec') &&
              dates[i].contains(dropdownvalueYear)) {
            spotsArray[11]++;
          }
        }
    ...
    if (dropdownvalueMonth != null && dropdownvalueMonth != "Alle") {
      ...
      daysInOneMonth = List<double>.filled(daysInMonth, 0,
          growable: false); //makes list for every day in month
  ...
  static List<String> getYears() {
    ...
    for (int i = 1; i < 10; i++) {
      items[i] =
          (yearInt - i).toString(); //gets all 20 years before current year
    }
