...
class _StatefulHydrantWidgetState extends State<StatefulHydrantWidget>{
    ...
    return Scaffold(
        ...
        Stack(
          ...
          FutureBuilder<List<Hydrant>>(
              future: loadMarkers(),
              builder: (BuildContext buildContext, AsyncSnapshot<List<Hydrant>> snapshot){
                return OSMFlutter(
  ...
  Future<List<Hydrant>> loadMarkers() async{
    ...
    var hydrants = await getHydrants(currentPostion.longitude, currentPostion.latitude);
  ...
  Future<http.Response> getHydrants(double lng, double lat) async{
    var uri = Uri.parse('https://api.wasserkarte.info/1.0/getSurroundingWaterSources/?source=$source&token=$token&lat=${lat.toStringAsFixed(2)}&lng=${lng.toStringAsFixed(2)}&range=$range&numItems=$numItems');
  ...
  Future<List<Hydrant>> convertHydrantResponse(String response) async{
    ...
    for(var singleWaterSource in waterSources){
      ...
      var myMarkerIcon = new MarkerIcon(icon: new Icon(Icons.location_pin, color: Colors.blue, size: 150));
      await controller.addMarker(new GeoPoint(latitude: newHydrant.lat!, longitude: newHydrant.lng!), markerIcon: myMarkerIcon);
    }
    return allHydrants;
  }
}