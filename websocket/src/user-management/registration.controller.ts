...
export class RegistrationController {
    ...
    async register(@Body() regUser:UserRegistrationDto){
        ...
        const r = await qRunner.query(`SELECT email,passwordhash,role,firestation FROM saferfireusers WHERE email LIKE '${regUser.email}' LIMIT 1`);
        if(r.length != 0){
            throw new HttpException('There is already a user with this email!', HttpStatus.BAD_REQUEST);
        }
        const res = await qRunner.query(`INSERT INTO saferfireusers (email,passwordhash,role,firestation) VALUES ('${regUser.email}','${regUser.passwordhash}','mannschaft','${regUser.firestation}')`);
        try{
